...
from Paths.paths import PATH_FOOTPATROL_FOLDER, MAIN_PATH
from os import path
...
def check_win(user_id, proxies, raffle_id, email, main_count, session, password):
    ...
    url = f'https://mosaic-platform.jdmesh.co/stores/footpatrolgb/user/{user_id}/preauth?api_key=6048110e2d7e087082d9a8d1c07b9e2c&channel=iphone-mosaic'
    ...
    if win_found == True:
        if win_response['orders'][count]['status'] == 'looser_processed':
            ...
            webhooks.footpatrol_winchecker_webhook(title, option_size, image, orderstatus, addressline1, addressline2, postcode, email, password, webhook)
            webhooks.astro_logs("Win Found!", "Footpatrol Launches")
...
def start():
    global amount_of_proxy_lines, twocaptchakey, retrylimit, webhook, PATH_PROXIES
    main_count = 1
