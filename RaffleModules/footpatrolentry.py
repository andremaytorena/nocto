...
import pandas as pd 
from Paths.paths import PATH_FOOTPATROL_LOGS, PATH_FOOTPATROL_FOLDER, MAIN_PATH
...
def write_success_logs(email):
    ...
    with open(PATH_FOOTPATROL_LOGS) as f:
            for line in f:
                pass
            has_newline = line.endswith('\n') or line.endswith('\n\r')

    with open(PATH_FOOTPATROL_LOGS, 'a', newline='') as f:
...
def discord_log_entry(email, size):
    ...
    webhooks.footpatrol_entry_webhook(raffle_name, email, size, image, webhook)
...
def identify3DS(redirect_url, md, paReq, pspReference, order_id, email, proxies, main_count, session, size):
    ...
    headers = {
        ...
        'Referer': 'https://footpatrolgb-mosaic-webapp.jdmesh.co/?channel=android-app-tablet-mosaic&appversion=2',
    ...
    payload = {
        'PaReq': paReq,
        'MD': md,
        'TermUrl': f'https://mosaic-platform.jdmesh.co/stores/footpatrolgb/preAuthorise/{order_id}/payment/3dsecure'
    }
    ...
    r = session.post(f'https://mosaic-platform.jdmesh.co/stores/footpatrolgb/preAuthorise/{order_id}/payment/3dsecure?isNewClient=1', headers=headers, data=payload)
    url = f'https://mosaic-platform.jdmesh.co/stores/footpatrolgb/preAuthorise/{order_id}/payment/3dsecure?api_key=0ce5f6f477676d95569067180bc4d46d&channel=android-app-tablet-mosaic'
    payload = {}
    ...
    try:
        ...
        headers = {
            ...
            'Origin': f'https://footpatrolgb-mosaic-webapp.jdmesh.co',
            'Connection': 'keep-alive',
            'User-Agent': "Mozilla/5.0 (Linux; Android 7.1.2; SM-G977N Build/LMY48Z; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/92.0.4515.131 Mobile Safari/537.36",
            'Referer': f'https://footpatrolgb-mosaic-webapp.jdmesh.co/?channel=android-app-tablet-mosaic&appversion=2'
        }
    ...
    for i in range(3):
        try:
            result = solver.recaptcha(
                ...
                url='https://footpatrolgb-mosaic-webapp.jdmesh.co',
                version='v3',
                action="Login/SignUp",
                score=0.9
            )
...
def enter_raffle(user_id, product_id, product_size_id, size, email, password, firstname, lastname, addressline1, addressline2, city, postcode, phonenumber, cardnumber, cardmonth, cardyear, cardcvv, proxies, main_count, session):
    ...
    for i in range(1):
        ...
        product_url = f"https://mosaic-platform.jdmesh.co/stores/footpatrolgb/user/{user_id}/preauth/product/{product_id}?api_key=6048110e2d7e087082d9a8d1c07b9e2c&channel=android-app-tablet-mosaic"
        product_response = session.get(product_url, proxies=proxies)

        enter_url = f"https://mosaic-platform.jdmesh.co/stores/footpatrolgb/preAuthorise/order?api_key=6048110e2d7e087082d9a8d1c07b9e2c&channel=android-app-tablet-mosaic&userID={user_id}"
        ...
        enter_raffle_response = session.post(enter_url, json=payload, proxies=proxies)
        ...
        put_url = f'https://mosaic-platform.jdmesh.co/stores/footpatrolgb/preAuthorise/payment/{order_id}?api_key=6048110e2d7e087082d9a8d1c07b9e2c&channel=android-app-tablet-mosaic&type=CARD'
...
def login(email, password, proxies, main_count, session):
    ...
    session.headers = {
        ...
        'origin': 'https://footpatrolgb-mosaic-webapp.jdmesh.co',
        'originalhost': 'mosaic-platform.jdmesh.co',
        'referer': 'https://footpatrolgb-mosaic-webapp.jdmesh.co/',
    ...
    login = session.post("https://mosaic-platform.jdmesh.co/stores/footpatrolgb/users/login?api_key=6048110e2d7e087082d9a8d1c07b9e2c&channel=android-app-tablet-mosaic", json=login_payload, proxies=proxies)
...
def start(size, email, password, firstname, lastname, addressline1, addressline2, city, postcode, phonenumber, cardnumber, cardmonth, cardyear, cardcvv, product_id, sizes_list, main_count):
    ...
    if ENTERED_RAFFLE  == True:
        ...
        os.system(f'title Footpatrol Launches Entry Module - Success: {success_entry_count} - Failed: {failed_entry_count}')
...
def set_csvs():
    ...
    choose_csv = list(filter(lambda x: '.csv' in x, os.listdir(PATH_FOOTPATROL_FOLDER)))
    ...
    option = int(input('Option: '))
    PROFILES_CSV = path.join(PATH_FOOTPATROL_FOLDER, choose_csv[option])
    print("")
    return PROFILES_CSV
...
def CheckEntryStatus(): # Opens accounts csv and gets account
    ...
    results_entered = pd.read_csv(PATH_FOOTPATROL_LOGS)
        ...
        for i in range(threads): #amount of threads
            ...
            for i in range(1): # amount of lines in accounts CSV
                ...
                for i in range(_linesentered): # amount of lines in entered CSV
                    ...
                    with open(PATH_FOOTPATROL_LOGS) as entered:
