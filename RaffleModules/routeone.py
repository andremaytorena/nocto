...
from Paths.paths import PATH_CHROME_DRIVER, PATH_SETTINGS, PATH_ROUTEONE_LOGS, PATH_PROXIES, PATH_ROUTEONE_FOLDER
...
def write_success_logs(email):
    ...
    with open(PATH_ROUTEONE_LOGS) as f:
            for line in f:
                pass
            has_newline = line.endswith('\n') or line.endswith('\n\r')

    with open(PATH_ROUTEONE_LOGS, 'a', newline='') as f:
...
def waiting_solved(stripe_url, email, main_count):
    ...
    while True:
        if 'routeone.com' in driver.current_url:
...
def checkout(size, email, password, firstname, lastname, addressline1, addressline2, city, postcode, phonenumber, cardnumber, cardmonth, cardyear, cardcvv, main_count, proxies):
    ...
    for i in range(3):
        ...
        for i in range(1):
            ...
            url = 'https://launches.routeone.co.uk/cart/add.js'

            headers = {
                "origin": "https://launches.routeone.co.uk",
                "referer": raffle_name,
            ...
            goingtocheckout_response = session.get('https://launches.routeone.co.uk/checkout', headers=headers, proxies=proxies)
            ...
            headers = {
                ...
                "origin": "https://launches.routeone.co.uk",
                "referer": "https://launches.routeone.co.uk",
            ...
            payload = {
                ...
                "payment_session_scope": "launches.routeone.com"
            }
...
def set_csvs():
    ...
    choose_csv = list(filter(lambda x: '.csv' in x, os.listdir(PATH_ROUTEONE_FOLDER)))
    ...
    PROFILES_CSV = path.join(PATH_ROUTEONE_FOLDER, choose_csv[option])
    print("")
    return PROFILES_CSV
...
def CheckEntryStatus(): # Opens accounts csv and gets account
    ...
    results_entered = pd.read_csv(PATH_ROUTEONE_LOGS)
        ...
        for i in range(threads): #amount of threads
            ...
            for i in range(1): # amount of lines in accounts CSV
                ...
                for i in range(_linesentered): # amount of lines in entered CSV
                    ...
                    with open(PATH_ROUTEONE_LOGS) as entered:
